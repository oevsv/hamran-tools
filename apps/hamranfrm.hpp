...
public:
  hrframegen(double sample_rate, double gain, std::size_t prefix_fraction,
             std::size_t phy_mode);
  virtual ~hrframegen();

  void assemble(const unsigned char *header, const unsigned char *payload,
...
typedef int(fn_framesync_callback)(unsigned char *, int, unsigned char *,

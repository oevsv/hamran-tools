...
keyer::keyer(float cpm, float f_0, unsigned handicap, float a, float f_s)
    : t_unit_(6.0 / cpm), handicap_(handicap), a_(a), f_s_(f_s),
      q_(1e-4) //(1.0-exp(-10.0f/(t_unit_*f_s)))
      ,
      o_(exp(2.0i * acos(-1) * double(f_0 / f_s))), g_(1.0), x_(conj(o_)),
      on_(true), n_t_(0) {}

void keyer::key_times(vector<pair<bool, float>> time_series) {
  copy(time_series.begin(), time_series.end(), back_inserter(tq_));
}

float keyer::pending_time() {
  float duration = 0;
  for (auto &t : tq_)
    duration += t.second;
  return duration;
...
void keyer::didah(const std::string &didah) {
...
void keyer::space(const space_t &s) {
  if (!tq_.empty()) {
    if (tq_.back().first)
...
void keyer::space(float duration) { tq_.push_back(make_pair(false, duration)); }

void keyer::mark(float duration) { tq_.push_back(make_pair(true, duration)); }

void keyer::send(const string &message) {
...
size_t keyer::get_frame(complex<float> *frame, size_t frame_size) {
  size_t count = 0;
  if (not tq_.empty()) {

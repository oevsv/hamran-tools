...
class keyer {
  ...
  std::deque<std::pair<bool, float>> tq_; // keyer time series
  std::deque<std::complex<float>> fb_;    // frame buffer
public:
  keyer(float cpm = 60 // characters per minute
        ,
  ...
  void didah(const std::string &didah);

  void space(const space_t &s);

  void space(float duration);

  void mark(float duration);

  void send(const std::string &message);

  std::size_t get_frame(std::complex<float> *frame, std::size_t frame_size);
};

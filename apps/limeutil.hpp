...
public:
  ...
  time_point now() noexcept {
    return duration(stream.GetHardwareTimestamp()) + sys_epoch;
  }

  lime::Timespec to_lime(const time_point &tp) noexcept {
    return lime::Timespec(duration(tp - sys_epoch).count());
  }
